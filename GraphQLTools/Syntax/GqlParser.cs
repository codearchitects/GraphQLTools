...
namespace GraphQLTools.Syntax
{
    internal ref struct GqlParser
    {
        ...
        private Span _anonymousOperationSpan;
        ...
        private void Parse()
        {
            ...
            {
                Span span = _lexer.Span;
                if (span.IsEmpty)
                {
                    span = new Span(span.Start, 1);
                }
        ...
        private bool Match(TokenKind kind, string[] values)
        {
            if (_lexer.Kind != kind)
                return false;
        ...
        private bool MatchButNot(TokenKind kind, string[] values)
        {
            if (_lexer.Kind != kind)
                return false;
        ...
        private void MoveNext()
        {
            while (_lexer.MoveNext())
            {
                switch (_lexer.Kind)
                {
                    ...
                    case TokenKind.Error:
                        throw new DiagnosticException(_lexer.ErrorMessage);
